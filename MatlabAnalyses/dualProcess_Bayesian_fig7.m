switch getenv('computername')
    case 'LMN'
        directory = 'C:\Users\Le\Dropbox (MIT)\Jazayeri\NoisyMutualInhibition';
    otherwise
        directory = 'C:\Users\Sur lab\Documents\Noisy_mutual_inhibition\NoisyMutualInhibition\';
end
...
save('dualProcess_RSG_fitting_013020.mat', 'all_subject_files', 'Iopt_lst',...
    'Kopt_lst', 'sigma_opt_lst');
...
behav_file = fullfile(directory, all_subject_files{id});
...
errorbar(tss + 5, mean(RSG_productions),...
    std(RSG_productions), 'Color', purple, 'Marker', 'o', 'LineStyle', 'none',...
    'MarkerFaceColor', purple, 'MarkerSize', 4,...
    'LineWidth', 1.5)
plot(tss + 5, mean(RSG_productions), 'Color', purple)
plot(tss, mtp_in(:,1), 'k')
xlim([500, 1100])
ylim([500, 1100])
xlabel('$t_s (ms)$')
...
l2 = errorbar(tss + 5, mean(RSSG_productions),...
    std(RSSG_productions), 'Color', purple, 'Marker', 'o', 'LineStyle', 'none',...
    'MarkerFaceColor', purple, 'MarkerSize', 4,...
    'LineWidth', 1.5);

l3 = plot(tss + 5, mean(RSSG_productions), 'Color', purple);
l4 = plot(tss, mtp_in(:,2), 'k');
...
%save('biasvar_dualProcess_013020c.mat', 'collated_bias_var', 'all_subject_files');
...
function lst = randrange(low, high, dim)
    lst = rand(dim) * (high - low) + low;
end